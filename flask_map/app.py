...
from markupsafe import escape # エスケープ処理

app = Flask(__name__)

# fromのファイル名に予約語や既存クラス名はつかえない
from map import map_module
...
def pgtable():
    ...
    #return render_template('tables.html', tables=rows, title='hoge')
    params = {
        'tables': rows,
        'title': 'pg_tables'
    }
    return render_template('tables.html', **params)
